...
def normalize_bait_node(bait: str) -> str:
    """Uses the bait code as the network node name."""
    bait_code = str(bait).split()[0]
    if bait_code == "SARS-CoV2":
        return "COVID19"
    return bait_code


def add_bait_dip_edges_to_graph(
    graph: nx.Graph,
    dip_proteins_by_bait: Dict[str, List[str]],
) -> int:
    """Adds Bait--DIP STRING ID edges to a final SIP graph."""
    bait_dip_edges = {
        (normalize_bait_node(bait), dip_id)
        for bait, dip_ids in dip_proteins_by_bait.items()
        for dip_id in dip_ids
    }
    graph.add_edges_from(bait_dip_edges)
    return len(bait_dip_edges)
...
def main():
    ...
    num_bait_dip_edges = add_bait_dip_edges_to_graph(sip_graph, dip_proteins_by_bait_in_graph)
    logging.info(f"Added {num_bait_dip_edges:,} Bait--DIP STRING ID edges to the final SIP graph.")
    ...
    # --- 7. Save Final Graph ---
    output_file_base = result_dir / f"SIP_network_combined_{combined_score_threshold}_with_bait_edges"
    try:
...
def read_network(config_path: str = "config.json") -> Optional[nx.Graph]:
    ...
    try:
        ...
        file_path = result_dir / f"SIP_network_combined_{combined_score}_with_bait_edges.pkl"
